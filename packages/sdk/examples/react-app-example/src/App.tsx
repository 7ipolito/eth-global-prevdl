...
async function ensureOasisNetwork(ethereum: any): Promise<void> {
  try {
    ...
    if (chainId !== OASIS_CHAIN_ID) {
      console.log(`🔄 Mudando rede de ${chainId} para Oasis Sapphire Testnet (${OASIS_CHAIN_ID})...`);
      ...
      try {
        ...
        if (switchError.code === 4902 || switchError.code === -32603) {
          console.log('➕ Adicionando rede Oasis Sapphire Testnet ao MetaMask...');
          await ethereum.request({
    ...
    throw new Error(`Failed to switch to Oasis Sapphire Testnet: ${error.message}`);
  }
}
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (typeof window.ethereum !== 'undefined') {
          try {
            ...
            if (Number(network.chainId) !== OASIS_CHAIN_ID_DECIMAL) {
              throw new Error(`Wrong network! Expected Oasis Sapphire Testnet (${OASIS_CHAIN_ID_DECIMAL}), but got chainId ${network.chainId}. Please switch networks in MetaMask.`);
            }
            
            console.log(`✅ Conectado à Oasis Sapphire Testnet (Chain ID: ${network.chainId})`);
            ...
            console.warn('   Tentando fallback para private key ou modo local...');
        ...
        if (!walletConnected) {
          ...
          if (privateKey) {
            ...
            console.warn('⚠️  MetaMask não encontrada e VITE_PRIVATE_KEY não configurada');
    ...
    if (typeof window.ethereum !== 'undefined') {
      const handleChainChanged = (chainId: string) => {
        ...
        if (chainId !== OASIS_CHAIN_ID) {
          console.warn(`⚠️  Rede alterada para ${chainId}. Por favor, mude para Oasis Sapphire Testnet (${OASIS_CHAIN_ID})`);
          setError(`Wrong network! Please switch to Oasis Sapphire Testnet (Chain ID: ${OASIS_CHAIN_ID_DECIMAL})`);
        } else {
          setError(null);
    ...
    return (
      <div style={{ 
        ...
        <p style={{ fontSize: '0.9rem', color: '#888', marginTop: '1rem' }}>
          Se estiver demorando, verifique se MetaMask está instalado e configurado para a rede Oasis Sapphire Testnet (Chain ID: 23295)
        </p>
        <p style={{ fontSize: '0.85rem', color: '#666', marginTop: '0.5rem' }}>
          Ou configure VITE_PRIVATE_KEY no .env para usar private key diretamente
        </p>
      </div>
    );
  }
